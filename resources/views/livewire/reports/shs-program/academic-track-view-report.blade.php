<div class="max-w-screen-xl mx-auto p-10 m-6 my-16">
        ...
        <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
            ...
            <li aria-current="page">
                <div class="flex items-center">
                    ...
                    <span class="ms-1 text-sm font-medium text-gray-500 md:ms-2 dark:text-gray-400">View | {{$academicTrack->strand}}</span>
                </div>
            </li>
        </ol>
    </nav>
        ...
        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
            ...
            <tbody>
                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                    ...
                    <td class="px-6 py-4">
                        {{ $academicTrack->school_year_start }}
                    </td>
                    <td class="px-6 py-4">
                        {{ $academicTrack->school_year_end }}
                    </td>
                    <td class="px-6 py-4">
                        {{ $academicTrack->strand }}
                    </td>
                    <td class="px-6 py-4">
                        {{ $academicTrack->grade_lvl}}
                    </td>
                </tr>
            </tbody>
        </table>
    ...
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg border mt-8">
        <div class="flex flex-column sm:flex-row flex-wrap space-y-4 sm:space-y-0 justify-center pb-4 p-4 bg-slate-600 text-slate-50 uppercase font-bold">
        ...
        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
            ...
            <tbody>
                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                    <td scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                        {{ $academicTrack->total_male_enrolled }}
                    </td>
                    <td class="px-6 py-4">
                        {{ $academicTrack->total_female_enrolled }}
                    </td>
                    <td class="px-6 py-4">
                        {{ $academicTrack->overall_enrolled }}
                    </td>
                    <td class="px-6 py-4">
                        {{ $academicTrack->total_graduates }}
                    </td>
                    <td class="px-6 py-4">
                        {{ $academicTrack->total_student_pursuing_college }}
                    </td>
                    <td class="px-6 py-4">
                        {{ $academicTrack->total_student_pursuing_college }}
                    </td>
                    <td class="px-6 py-4">
                        {{ $academicTrack->total_student_seeking_job }}
                    </td>
                    <td class="px-6 py-4">
                        {{ $academicTrack->undecided_student_total }}
                    </td>
                </tr>
            </tbody>
        </table>
    ...
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg border mt-8">
        @if(empty($academicTrack->total_dost_passer ))
            <div class="flex flex-column sm:flex-row flex-wrap space-y-4 sm:space-y-0 justify-center pb-4 p-4 bg-slate-600 text-slate-50 uppercase font-bold">
                <p>NO DOST Passers</p>
            </div>
        @else
            <div class="flex flex-column sm:flex-row flex-wrap space-y-4 sm:space-y-0 justify-center pb-4 p-4 bg-slate-600 text-slate-50 uppercase font-bold">
                <p>DOST Passers Lists</p>
            </div>
            <div
                ...
                @if(Auth::user()->role == 'admin')
                   <div>
                        ...
                        <div id="dropdownAction"
                            ...
                            <div class="py-1">
                                <a href="#" wire:click.prevent='deleteSelectedDostPassers'
            ...
            <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                ...
                <tbody>
                    @foreach ($dostPassers as $dostPasser)
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                        @if(Auth::user()->role == 'admin')
                            <td class="w-4 p-4">
                                <div class="flex items-center">
                                    <input id="checkbox-table-search-{{ $dostPasser->id }}" type="checkbox"
                                        class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 dark:focus:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600"
                                        wire:model="selectedDostPassers" value="{{ $dostPasser->id }}">
                                    <label for="checkbox-table-search-{{ $dostPasser->id }}" class="sr-only">checkbox</label>
                                </div>
                            </td>
                        @endif
                        <td scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                            {{ $dostPasser->full_name }}
                        </td>
                        @if(Auth::user()->role == 'admin')
                            <td class="px-6 py-4">
                                <!-- Modal toggle -->
                                <a href="#" type="button" data-modal-target="editUserModal" wire:click.prevent='EditFullName({{$dostPasser->id}})'
           ...
           <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
            ...
            <div id="editUserModal" tabindex="-1" aria-hidden="true"
                ...
                <div class="relative w-full max-w-2xl max-h-full">
                    <!-- Modal content -->
                    <form class="relative bg-white rounded-lg shadow dark:bg-gray-700" wire:submit.prevent='updateDOSTPasserName'>
                        <!-- Modal header -->
