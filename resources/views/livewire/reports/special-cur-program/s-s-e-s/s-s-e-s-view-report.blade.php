            ...
            <li aria-current="page">
                <div class="flex items-center">
                    ...
                    <span class="ms-1 text-sm font-medium text-gray-500 md:ms-2 dark:text-gray-400">View |
                       Special Science Elementary School Program</span>
                </div>
            </li>
        </ol>
    </nav>
        ...
        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
            ...
            <tbody>
                <tr
                    ...
                    <td class="px-6 py-4">
                        {{ $SSESProgram->school_year_start }}
                    </td>
                    <td class="px-6 py-4">
                        {{ $SSESProgram->school_year_end }}
                    </td>
                    <td class="px-6 py-4">
        ...
        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
            <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                <tr class="p-4">
                    ...
                    @if ($SSESProgram->grade_lvl === 'Grade 6')
                        <th scope="col" class="px-6 py-3">
                            Pisay Passers
                        </th>
                        <th scope="col" class="px-6 py-3">
            ...
            <tbody>
                <tr
                    class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                    <td scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                        {{ $SSESProgram->no_enrolled_male_stud }}
                    </td>
                    <td class="px-6 py-4">
                        {{ $SSESProgram->no_enrolled_female_stud }}
                    </td>
                    <td class="px-6 py-4">
                        {{ $SSESProgram->overall_enrolled }}
                    </td>
                    @if($SSESProgram->grade_lvl === 'Grade 6')
                        <td class="px-6 py-4">
                            {{ $SSESProgram->pisay_passers }}
                        </td>
                        <td class="px-6 py-4">
                            {{ $SSESProgram->ste_passers }}
                        </td>
                    @endif
                </tr>
            </tbody>
