<div>
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg border mt-8">
        ...
        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
            ...
            <tbody>
                @foreach ($SPEDPrograms as $SPEDProgram)
                <tr
                    class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                    <td scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                        {{$SPEDProgram->school->school_id}}
                    </td>
                    <td class="px-6 py-4">
                        {{$SPEDProgram->school->name}}
                    </td>
                    <td class="px-6 py-4">
                        {{$SPEDProgram->type_of_learners}}
                    </td>
                    <td class="py-4">
                        <div class="flex flex-wrap {{ Auth::user()->role == 'admin' ? '' : 'pl-8'}}">
                            <a href="#" wire:click="navigateTo('special-C-program/sped-view',{{$SPEDProgram->id}})"
                                class="font-medium dark:text-blue-500 hover:underline mr-4 hover:text-slate-400 text-gray-600"><svg
                             ...
                             @if (Auth::user()->role == 'admin')
                                <a href="#" wire:click="navigateTo('special-C-program/sped-edit',{{$SPEDProgram->id}})"
                                ...
                                <a href="#" @click.prevent="confirmDelete({{$SPEDProgram->id}}, '{{$deleteIdentifier}}')"
