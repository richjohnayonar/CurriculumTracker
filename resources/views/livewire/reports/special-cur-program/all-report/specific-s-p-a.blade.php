<div class="max-w-screen-xl mx-auto p-10 m-6 my-16">
    ...
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg border mt-10 bg-gray-200">
        ...
        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
            ...
            <tbody>
                @foreach ($SPAByGradeLvls as $SPAByGradeLvl)
                <tr
                    class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                    <td scope="row" class="px-6 py-4">
                        {{ $SPAByGradeLvl->school->school_id }}
                    </td>
                    <td class="px-6 py-4">
                        {{ $SPAByGradeLvl->school->name }}
                    </td>
                    <td class="px-6 py-4">
                        {{ $SPAByGradeLvl->school_year_start }}
                    </td>
                    <td class="px-6 py-4">
                        {{ $SPAByGradeLvl->school_year_end }}
                    </td>
                    <td class="px-6 py-4">
                            {{ $SPAByGradeLvl->grade_lvl }}
                        </td>
                    <td class="px-6 py-4">
                        {{ $SPAByGradeLvl->no_enrolled_male_stud }}
                    </td>
                    <td class="px-6 py-4">
                        {{ $SPAByGradeLvl->no_enrolled_female_stud }}
                    </td>
                    <td class="px-6 py-4">
                        {{ $SPAByGradeLvl->overall_enrolled }}
                    </td>
                </tr>
                @endforeach
            </tbody>
